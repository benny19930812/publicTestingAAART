	...
	<c:forEach var="orderCo" varStatus="stat" items="${ccc.cartCo}"> <!-- 兩層 ccc.cartCo 在 CourseControllerF #43 #71 -->
		...
		<!--如果是HashMap，var後需要加上  .value 。List則不用-->
			...
			<td><fmt:formatNumber value="${orderCo.value.coNum * orderCo.value.coPrice}"
					type="number" />元</td>
			<td>${orderCo.value.coAct_Date}</td>	
			<td>${orderCo.value.coAct_Time}</td>		
			<td><input type="button" value="刪除本課程"
			onclick="confirmDelete(${orderCo.value.coId})"></td>
		</tr>
	</c:forEach>
	<tr>
		<td colspan="4"><a href="<c:url value='/18/cSelectAllFront.ctrl' />">繼續購買課程</a></td>
		<td colspan="3">總計：<fmt:formatNumber value="${ccc.total}"
				type="number" />元
		</td>
		<td colspan="3"><a href="<c:url value='/18/SubmitCartCo.ctrl' />" onClick="return Checkout(${carList.subtotal});">送出訂單</a></td>
